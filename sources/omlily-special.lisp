...
(defmethod cons-lily-expr ((self om::poly) listclefs freestore)
  (let ((rep (list " ")) 
    ...
    (loop for staff in voices
          ...
          (setf rep (append rep (cons-lily-expr staff (nth i listclefs) freestore)))))
...
(defmethod cons-lily-expr ((self om::voice) clef freestore)
  ...
  (let ((rep (list (format nil "~s=" (cassq *voice-num* *voice-rank* )) "{" 
    ...
    (loop for mes in mesures
          for i = 1 then (+ i 1) do
          (setf rep (append rep (cons-lily-expr mes lastmes freestore)))
          (setf lastmes mes))
    (setf rep (append rep (list (format nil"} ~% ~%"))))
    rep))
...
(defmethod cons-lily-expr ((self om::measure) lastmes chiffrage)
  ...
  (let* ((inside (om::inside self))
    ...
    (unless (and lastmes (equal (first tree) (first (tree lastmes))))
      ;(setf *mesure-num* (incf *mesure-num*))
      ;(setf rep (list (format nil  "\%%%%%%%%%%%%%%%%%%%%%%% MESURE : ~d %%%%%%%%%%%%%%%%%%%%%%%" *mesure-num*)))
      (setf rep (append rep (list (format nil  "\\time ~d/~d" (om::fnumerator (first tree)) (om::fdenominator (first tree))))))
      (setf rep (append rep (list (format nil  "\\set Staff.timeSignatureFraction = #'(~d . ~d)" 
                                          (om::fnumerator (first tree)) (om::fdenominator (first tree))))))
      )
    ...
    (loop for obj in inside do
          (setf rep (append rep 
                            (let* ((dur-obj-noire (/ (om::extent obj) (om::qvalue obj)))
                                   ...
                                   (exp (cons-lily-expr obj (* symb-beat-val factor) (car (tree self)))))
                              exp
                              )
                            )))
    ;(setf *mesure-num* (incf *mesure-num*))
...
(defmethod cons-lily-expr ((self om::group) dur ratio)
     ...
     ((not (om::get-group-ratio self)) 
      (loop for obj in inside
            do (setf rep (append rep (let* ((dur-obj (/ (/ (om::extent obj) (om::qvalue obj)) 
                                       ...
                                       (cons-lily-expr obj (* dur-obj durtot) ratio))))))
     ((= sympli 1)
      (loop for obj in inside
            do (setf rep (append rep (let* ((operation (/ (/ (om::extent obj) (om::qvalue obj)) 
                                       ...
                                       (cons-lily-expr obj (* dur-obj unite) ratio)))))
...
|#
          ...
          (setf rep (append rep (list (format nil "\\times ~d/~d {" den numer))))
          )
        (loop for obj in inside do
              (setf rep (append rep (let* ((operation (/ (/ (om::extent obj) (om::qvalue obj)) 
                                      ...
                                      (setf tmp (multiple-value-list 
                                                 (cons-lily-expr obj (list (* dur-obj unite) cpt) ratio)))
                                      (setf exp (car tmp))
...
(defmethod cons-lily-expr ((self om::chord) dur ratio)
    (let* ((durtot (if (listp dur) (car dur) dur))
        (inside (om::inside self))
        (str ""))
    ...
    (if (>= durtot 16)
      ...
      (let* ((durconv (get-head-and-points durtot))
             ...
             (ratio-ord (if (powerof2? (second ratio))
                           1/4
                           (/ (/ (find-beat-symbol (second ratio)) 4) (second ratio))))
             (mutl  (if (powerof2? (second ratio))
                           (/ (car durconv) 2)
                           (* (/ (car durconv) 2) (/ (/ (find-beat-symbol (second ratio)) 4) (second ratio)))))
      ...
      (let* ((durconv (get-head-and-points durtot))
        ...
        (setf str (string+ str 
                           (if (not points)
                             (format nil "~d*~d" head mutlratio)
                             (format nil "~d~A*~d" head points mutlratio)
                             )
                           )))
...
(defmethod cons-lily-expr ((self om::rest) dur ratio)
    ...
    (prog1
      (if (>= durtot 16)
        ...
        (let ((mutlratio  (if (powerof2? (second ratio))
                            ...
                            (/ (/ (find-beat-symbol (second ratio)) 4) (second ratio)))))
          (if (not points)
            (setf str (string+ str  (format nil "r~d*~d" head mutlratio)))
            (setf str (string+ str  (format nil "r~d~A*~d" head points mutlratio)))
            ))
...
(defmethod! om->lily-spec ((self poly) &optional 
            ...
            (let* ((ressource-folder (lib-resources-folder (find-library "omlily")))
                   ...
                   (lilyfile (write-lil-file (cons-lily-expr self clef nil) pathname paperfile layoutfile)))
